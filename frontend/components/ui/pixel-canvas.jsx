...
// A single animated pixel "cell" inside the canvas.
class Pixel {
  ...
  draw() {
    ...
    const centerOffset = (this.maxSizeInteger * 0.5 - this.size * 0.5) * dpr
...
// The underlying web component that owns the <canvas> and animation loop.
class PixelCanvasElement extends HTMLElement {
  ...
  // When present, the canvas reveals itself once on mount (page-load
  // style reveal) instead of waiting for hover/focus, and it does not
  // attach the hover/focus listeners that would make it disappear again.
  get autoPlay() {
  ...
  connectedCallback() {
    ...
    requestAnimationFrame(() => {
      ...
      if (this.autoPlay) {
        // Reveal once, immediately, and let it shimmer ambiently forever.
...
// React wrapper around the <pixel-canvas> web component.
const PixelCanvas = React.forwardRef(
  ({ gap, speed, colors, variant, noFocus, autoPlay, style, ...props }, ref) => {
    React.useEffect(() => {
